...
val MIGRATION_1_2 =
    object : Migration(1, 2) {
        ...
        /**
         * Rebuilds the schema left by the old Capacitor/React app: converts
         * `durationHours` (REAL) to `durationMinutes` (INTEGER ×60), adds the
         * new required columns with safe defaults, and backfills
...
val MIGRATION_2_3 =
    object : Migration(2, 3) {
        ...
        /**
         * Creates the day_journal_entries, day_journal_responses,
         * app_settings and scheduled_notifications tables introduced in v3.
         */
        override fun migrate(database: SupportSQLiteDatabase) {
...
val MIGRATION_3_4 =
    object : Migration(3, 4) {
        ...
        /**
         * Adds the scoring_config table that stores the configurable scoring
         * weights used by the score engine.
         */
        override fun migrate(database: SupportSQLiteDatabase) {
...
val MIGRATION_4_5 =
    object : Migration(4, 5) {
        ...
        /**
         * Recurrence redesign: adds tasks.currentScore (decaying score),
         * tasks.lastOccurrenceDate, tasks.dayBoundaryHour and
         * task_occurrences.autoGenerated for system-created missed entries.
         */
        override fun migrate(database: SupportSQLiteDatabase) {
...
val MIGRATION_5_6 =
    object : Migration(5, 6) {
        ...
        /**
         * Adds actual-completion tracking to task_occurrences:
         * actualCompletedAt and actualDurationMinutes, capturing real vs
         * planned effort.
         */
        override fun migrate(database: SupportSQLiteDatabase) {
...
val MIGRATION_6_7 =
    object : Migration(6, 7) {
        ...
        /**
         * Adds focus-tracking columns to time_entries: focusRating,
         * focusNote and focusRatedAt, so users can rate how focused each
         * tracked span was.
         */
        override fun migrate(database: SupportSQLiteDatabase) {
...
val MIGRATION_7_8 =
    object : Migration(7, 8) {
        ...
        /**
         * Introduces the life_dimensions taxonomy (seeded with defaults +
         * Unassigned), the user_preferences typed-settings table, and the
         * dimension_id columns with backfill and indexes on the related
         * tables.
         */
        override fun migrate(database: SupportSQLiteDatabase) {
...
val MIGRATION_16_17 =
    object : Migration(16, 17) {
        ...
        /**
         * Habit-recurrence overhaul: drops task_occurrences.autoGenerated
         * (absence now implies missed), and rewrites every recurrenceRule from
         * the old CONFIG:/RRULE: encodings into the compact "num/den" format.
         * Rebuilds affected tables because SQLite < 3.35 lacks DROP COLUMN.
         */
        override fun migrate(database: SupportSQLiteDatabase) {
        ...
        private fun convertRecurrenceRules(database: SupportSQLiteDatabase) {
            ...
            /**
             * Normalizes a legacy `CONFIG:` recurrence string (pipe-separated
             * key=value pairs describing the cadence type) into the compact
             * "num/den" ratio used by the new recurrence model.
             */
            ...
            /**
             * Normalizes a legacy RFC-5545 `RRULE:` string (FREQ/INTERVAL/BYDAY/
             * BYMONTHDAY) into the compact "num/den" ratio used by the new
             * recurrence model (e.g. WEEKDAYS BYDAY → "5/7").
             */
...
val MIGRATION_17_18 =
    object : Migration(17, 18) {
        ...
        /**
         * Creates the three self-governance metric tables — habit_metrics (L1),
         * dimension_metrics (L2) and day_metrics (L3) — as the foundation for
         * the score roll-up. SQL-only; rule conversion and backfill happen in
         * the post-open backfill service.
         */
        override fun migrate(database: SupportSQLiteDatabase) {
...
val MIGRATION_18_19 =
    object : Migration(18, 19) {
        ...
        /**
         * Drops the decaying `currentScore` column from tasks (the score
         * roll-up replaced decay; consumers now read L1 metrics directly).
         * Rebuilds the tasks table since SQLite < 3.35 lacks DROP COLUMN.
         */
        override fun migrate(database: SupportSQLiteDatabase) {
...
val MIGRATION_19_20 =
    object : Migration(19, 20) {
        ...
        /**
         * Adds the user-editable `weight` column to life_dimensions (NOT NULL
         * DEFAULT 1.0 keeps legacy rows equally weighted); L3 day scores become
         * a weighted average once weights are set.
         */
        override fun migrate(database: SupportSQLiteDatabase) {
...
val MIGRATION_20_21 =
    object : Migration(20, 21) {
        ...
        /**
         * Guards task_occurrences against duplicate (taskId, day) rows: dedupes
         * existing twins (keeping the user row over auto-missed), then adds a
         * unique index on (taskId, date(dueDate)) so the auto-miss path can no
         * longer shadow a user-marked completion.
         */
        override fun migrate(database: SupportSQLiteDatabase) {
